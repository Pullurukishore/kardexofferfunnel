...
export default function NewOfferPage() {
  ...
  // Initialize zone for zone user: prefer /me API, fallback to AuthContext
  useEffect(() => {
    const initZone = async () => {
      try {
        setLoadingZones(true)
        const me = await apiService.getMe()
        const u: any = (me && (me as any).user) ? (me as any).user : me
        const zoneFromArray = Array.isArray(u?.zones) && u.zones.length > 0 ? u.zones[0] : null
        const zoneId = zoneFromArray?.id ?? u?.zoneId
        const zoneName = zoneFromArray?.name ?? u?.zone?.name
        if (zoneId) {
  ...
  // Also react to AuthContext updates (fallback)
  useEffect(() => {
    if (!user) return
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Select value={formData.zoneId} onValueChange={(value) => handleInputChange('zoneId', value)} disabled={zones.length === 0}>
                  <SelectTrigger>
                    <SelectValue placeholder={zones.length === 0 ? "No zones assigned to your account" : "Select service zone"} />
                  </SelectTrigger>
                  <SelectContent>
                    {zones.length > 0 ? (
                      zones.map(zone => (
